...
## ⚠️ 讨论/多 Agent 控制配置（多群聊 Agent 必读）

如果群聊中有多个 AI Agent（如金融助理 + 宏观助理），需要配置**讨论终止标记**防止无限循环回复。
...
### 2. ⚠️ 在 SOUL.md 中添加使用说明

**插件无法自动修改你的 SOUL.md，请手动添加以下内容：**

```markdown
## 讨论控制

在群聊中与其他 AI 讨论时，使用以下标记控制讨论流程：

| 标记 | 使用场景 | 示例 |
|------|----------|------|
| `【共识达成】` | 你认为双方观点已经一致，无需继续讨论 | "我同意你的看法。【共识达成】" |
| `ℹ️` | 你只是提供信息，不是在讨论或辩论 | "以下是相关数据。ℹ️" |

**重要**：这些标记会让其他 AI 停止回复，避免无限讨论循环。请在合适的情况下主动使用。
```
...
| 标记 | 含义 | 触发效果 |
|------|------|----------|
| `【共识达成】` | AI 认为观点一致，无需继续讨论 | 其他 AI 不回复 |
| `ℹ️` | AI 仅提供信息，不是在讨论 | 其他 AI 不回复 |
